## W75: EventModal「現場案件として登録」UI完全削除 (2026-04-14)

- 担当: くろさん（W75）
- 前ワーカーが state 削除のみで停止、UI・payload が残ってビルドエラー状態だった
- 削除内容:
  - `SiteInfo` import 削除
  - `site` 変数（editing?.site 引き継ぎコード）削除
  - `baseBody` から `site` フィールド削除
  - 現場案件UIセクション全体削除（チェックボックス/売値/原価/粗利表示/現場情報textarea）
- DBの既存 site データは保持（画面から編集不可になるだけ）
- ビルド確認OK → コミット a11e3da → push origin main 完了
