...
export async function GET(req: NextRequest) {
  ...
  if (!raw) return NextResponse.json({ q: '', products: [], customer: { id: user.customer_id, company: user.company } });
  ...
  const products = hits.map((r) => {
    ...
    const hbU = String(r.hinban || '').toUpperCase();
  ...
  // 顧客に見せても無意味なゴミ品番(上代も価格も無い=HP販売v3由来の重複等)は除外する。
  // 健太郎さん2026-06-17「ホームページ由来のわけ分からん品番(SH2PTWD等)が怖い」対応。
  .filter((p) => p.jodai_m2 != null || p.customer_meter_tanka != null || p.customer_meter_tanka_new != null);
